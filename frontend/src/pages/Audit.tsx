...
import { fetchAuditHistory, pollAuditStatus, setCurrentAudit, setPolling, startAudit } from '../store/slices/auditSlice'
...
const Audit = () => {
  ...
  const auditUid = searchParams.get('uid')
  ...
  const updateQueryParams = (nextUid?: string) => {
  ...
  useEffect(() => {
    ...
    void dispatch(fetchAuditHistory(projectId ? Number(projectId) : undefined))
  }, [dispatch, isAuthenticated, projectId])

  useEffect(() => {
    if (!auditUid || currentAudit?.uid === auditUid) {
      return
    }

    dispatch(setPolling(true))
    void dispatch(pollAuditStatus(auditUid))
  }, [auditUid, currentAudit?.uid, dispatch])
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void dispatch(pollAuditStatus(currentAudit.uid))
    }, 2000)
  ...
  const handleStartAudit = async (event: React.FormEvent) => {
    ...
    const action = await dispatch(startAudit({ url, projectId: projectId ? Number(projectId) : undefined }))

    if (startAudit.fulfilled.match(action)) {
      updateQueryParams(action.payload.uid)
      setUrl('')
    }
  }
        ...
        <div className="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="flex items-center gap-3">
              <Link to="/" className="text-2xl font-bold text-blue-600">SEO Master</Link>
              <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-blue-700">
                Public Primary Audit
              </span>
            </div>
            <h1 className="mt-4 text-3xl font-bold text-gray-900">Site audit</h1>
            <p className="mt-2 max-w-3xl text-gray-600">
              Run a technical SEO audit without registration and share the result by direct link. If you sign in, the same workspace also shows your private audit history and HITL approvals.
            </p>
          </div>

          {!isAuthenticated && (
            <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
              Need saved history and approvals? <Link to="/login" className="font-semibold underline">Sign in</Link>.
            </div>
          )}
        </div>
        ...
        <Card className="p-6">
          ...
          {auditUid && (
            <p className="mt-3 text-sm text-gray-500">
              Direct result link: <span className="font-medium text-gray-700 break-all">{window.location.href}</span>
            </p>
          )}
        </Card>
                ...
                <div key={audit.uid} className="flex flex-col gap-3 rounded-xl bg-gray-50 p-4 transition-colors hover:bg-gray-100 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <Button
                      type="button"
                      onClick={() => {
                        dispatch(setCurrentAudit(audit))
                        updateQueryParams(audit.uid)
                      }}
                    >
                      View
                    </Button>
